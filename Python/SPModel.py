...
class SPZigbeeDevice:
    ...
    id_: str            # Identify device among many devices of same type. Use "friendly name"
    type_: str          # Type of device i.e. pir, led or power plug
    LED_friend : str = None         # LED must blink in the room were user was most recently detected. Associate PIR's with an LED to achieve this.
...
class SPModel:
    ...
    @property
    def actuators_list(self) -> List[SPZigbeeDevice]:
        return list(filter(lambda s: s.type_ in {"led", "power plug"},
                           self.__devices.values()))

    @property
    def devices_list(self) -> List[SPZigbeeDevice]:
        return list(self.__devices.values())

    @property
    def sensors_list(self) -> List[SPZigbeeDevice]:
        return list(filter(lambda s: s.type_ in {"pir"},
                           self.__devices.values()))

    def add(self, device: Union[SPZigbeeDevice, List[SPZigbeeDevice]]) -> None:
        """ Add a new devices to the database.

        Args:
            device (Union[SPZigbeeDevice, List[SPZigbeeDevice]]): a device object, or a list of
            device objects to store.
        """
        # If the value given as argument is a SPZigbeeDevice, then create a list with it so that
        # later only a list of objects has to be inserted.
        list_devices = [device] if isinstance(device, SPZigbeeDevice)\
            else device
    ...
    def find(self, device_id: str) -> Optional[SPZigbeeDevice]:
        ...
        Returns:
            Optional[SPZigbeeDevice]: a device. If the device is not stored, then None is returned
        """
