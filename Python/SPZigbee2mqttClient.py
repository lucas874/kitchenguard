# This file is written by Jorge Miranda made available on blackboard. Renamed
...
class SPZigbee2mqttMessageType(Enum):
...
class SPZigbee2mqttMessage:
    ...
    # can be observed in the parse() method, where instances of SPZigbee2mqttMessage are created
    ...
    topic: str
    type_: SPZigbee2mqttMessageType
    data: Any = None
    event: Any = None
    message: Any = None
    meta: Any = None
    ...
    @classmethod
    def parse(cls, topic: str, message: str) -> SPZigbee2mqttMessage:
        ...
        Returns:
            SPZigbee2mqttMessage: an object with the parsed message values
        """
        # A note about class methods: these methods can be used to instantiate the class where it is
        # declared. In this case, this method returns an instance of SPZigbee2mqttMessage based on
        ...
        if topic == "zigbee2mqtt/bridge/state":
            instance = cls(type_=SPZigbee2mqttMessageType.BRIDGE_STATE,
                           topic=topic,
                           state=message)
        elif topic in ["zigbee2mqtt/bridge/event", "zigbee2mqtt/bridge/logging"]:
            type_ = {"zigbee2mqtt/bridge/event": SPZigbee2mqttMessageType.BRIDGE_EVENT,
                     "zigbee2mqtt/bridge/log": SPZigbee2mqttMessageType.BRIDGE_LOG}.get(topic)
            ...
            instance = cls(type_=SPZigbee2mqttMessageType.DEVICE_EVENT,
                           topic=topic,
...
class SPZigbee2mqttClient:
    ...
    def __init__(self,
                 host: str,
                 on_message_clbk: Callable[[Optional[SPZigbee2mqttMessage]], None],
                 port: int = 1883,
    ...
    def change_color(self, device_id: str, r, g, b) -> None:
        if not self.__connected:
            raise RuntimeError("The client is not connected. Connect first.")

        self.__client.publish(topic=f"zigbee2mqtt/{device_id}/set",
                              payload=json.dumps({"color": {"r": r, "g": g, "b": b}}))
    ...
    def __worker(self) -> None:
        ...
        while not self.__stop_worker.is_set():
            try:
                ...
                if message:
                    self.__on_message_clbk(SPZigbee2mqttMessage.parse(message.topic,
